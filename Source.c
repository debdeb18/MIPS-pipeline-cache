...
int search_cache(int addr) {
	...
	int index = block_addr % 256;
#ifdef CACHE_DEBUG
	printf("cache set %d\n", index);
#endif // CACHE_DEBUG

	int tag = bit(block_addr, 22, 11);
	int block_offset = bit(addr, 2, 1);
	...
	for (int i = 0; i < 4; i++) {
		if (cache[index].valid[i] && (cache[index].tag[i] == tag)) {
			...
			printf("cache hit\n");
#endif // CACHE_DEBUG
			return cache_mux(cache[index].data, i * 4, block_offset);
		}
	}

	//cache miss
	drain_cache(index, block_addr);
	return fill_cache(index, block_addr, tag, block_offset);
}

void write_cache(int addr, int input) {
	int block_addr = addr / 4;
	int index = block_addr % 256;
#ifdef CACHE_DEBUG
	printf("cache set %d\n", index);
...
#endif // CACHE_DEBUG
			cache[index].dirty[i] = 1;
			cache[index].data[(i * 4) + block_offset] = input;
			return;
		}
	}
	...
	//write miss
	drain_cache(index, block_addr);
	int ct = cache[index].ct;

	//update cache
	cache[index].valid[ct] = 1;
	cache[index].dirty[ct] = 1;
	cache[index].tag[ct] = tag;
	cache[index].data[(ct * 4) + block_offset] = input;
	ct_update(index);
}

void drain_cache(int index, int b_addr) {
#ifdef CACHE_DEBUG
	printf("cache write miss\n");
#endif // CACHE_DEBUG
	//look for empty spot
	for (;;) {
		if (cache[index].valid[cache[index].ct] == 0) {
			//printf("cache empty spot found\n");
			cold_miss++;
			return;
		}
	}

	//if set is full, look for oldest and not accessed
	conf_miss++;
	for (;;) {
		if (cache[index].sca[cache[index].ct] == 1) {
			cache[index].sca[cache[index].ct] = 0;
			ct_update(index);
			//printf("cache old but accessed found\n");
		}
		else {
			//printf("cache old but not accessed found\n");
			break;
		}
	}
	...
	if (cache[index].dirty[cache[index].ct] == 1) {
		int addr = (b_addr << 2);
		//update to memory
		for (int i = 0; i < 4; i++) {
			data_memory[addr | i] = cache[index].data[(ct * 4) + i];
		}
		data_memory[ex_mem_l.alu_out >> 2] = ex_mem_l.rt;
		//printf("cache consistency checked\n");
	} 

	cache[index].valid[ct] = 0; 
	cache[index].tag[ct] = 0;
	cache[index].dirty[ct] = 0;
	memset(cache[index].data[ct * 4], 0, 4);
	cache[index].sca[ct] = 0;
}

int fill_cache(int index,  int addr, int tag, int bo) {
#ifdef CACHE_DEBUG
	printf("cache filled with data from block address 0x%x\n", addr);
#endif // CACHE_DEBUG
	...
	cache[index].tag[ct] = tag;
	for (int i = 0; i < 4; i++) {
		cache[index].data[(ct * 4) + i] = data_memory[(addr << 2) | i];
#ifdef CACHE_DEBUG
		printf("set %d, data[%d] = 0x%x --> mem[%x]\n", index, (ct * 4) + i, data_memory[(addr << 2) | i], (addr << 2) | i);
#endif // CACHE_DEBUG
	}

	int res = cache_mux(cache[index].data, ct * 4, bo);
	ct_update(index);
	return res;
}

void ct_update(int index) {
	cache[index].ct++;
	if (cache[index].ct == 4) {
		cache[index].ct = 0;
	}
...
void memory_access(int* ALUOutM) {
	*ALUOutM = ex_mem_l.alu_out;
	...
	if (memWrite == 1) {
#ifdef STAGES_CHANGE
		printf("[%d] updated in cache\t --> 0x%x: 0x%x\n", ex_mem_l.i, ex_mem_l.alu_out, ex_mem_l.rt);
#endif
		write_cache(ex_mem_l.alu_out >> 2, ex_mem_l.rt);
	}
	else if (memRead == 1) {
		read_data = search_cache(ex_mem_l.alu_out / sizeof(unsigned int));
#ifdef STAGES_CHANGE
...
//mux in cache
int cache_mux(int data[16], int index, int s) {
	return data[index + s];
}
...
int print_output() {
	...
	double clk_cycle = (double)clk_t / CLOCKS_PER_SEC;
	double mem_time = (double)clk_t * 1000 / CLOCKS_PER_SEC;
	int c_miss = cold_miss + conf_miss;
	double miss_rate = ((double)c_miss/((double)c_hit + (double)c_miss));
	printf("Total cache hits\t\t --> %d hit\n", c_hit);
	printf("Total cache misses\t\t --> %d : %d cold miss, %d conflict miss\n", c_miss, cold_miss, conf_miss);
	printf("Assuming that the cache access time is %f secs, and memory access time is %f secs\n", clk_cycle, mem_time);
	printf("The Average Memory Access time is %f secs\n", clk_cycle + (mem_time * miss_rate));
#ifdef DEBUG
