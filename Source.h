...
unsigned int k, clk;
unsigned int c_hit, cold_miss, conf_miss;
...
//4-way set associativity --> 4 block line per sets
struct cacheline {
	unsigned int ct; //cache_tracker - recently written + 1, must be < 4, unless back to 0
	unsigned int tag[4]; 
	unsigned int sca[4];
	unsigned int valid[4];
	unsigned int dirty[4];
	int data[16]; //block size = 4 words
};
typedef struct cacheline Cache;
Cache cache[1024 / 4]; //256 sets
...
int fill_cache(int index, int nl, int addr, int tag);
